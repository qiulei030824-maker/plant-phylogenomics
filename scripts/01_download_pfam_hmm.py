...
Usage:
    python scripts/01_download_pfam_hmm.py <PFAM_ID>
    python scripts/01_download_pfam_hmm.py <PFAM_ID> -i /path/to/genomes -o /path/to/output
    python scripts/01_download_pfam_hmm.py <PFAM_ID> --hmmsearch /usr/bin/hmmsearch --force-redownload

Examples:
    python scripts/01_download_pfam_hmm.py PF00168                          # default paths
    python scripts/01_download_pfam_hmm.py PF00168 -i /data/genomes -o /data/results
    python scripts/01_download_pfam_hmm.py PF00168 --skip-hmmsearch         # HMM only
    python scripts/01_download_pfam_hmm.py C2
...
# ── Path setup: allow import from config/ directory ─────────────────────────
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_REPO_ROOT = os.path.dirname(_SCRIPT_DIR)
if _REPO_ROOT not in sys.path:
    sys.path.insert(0, _REPO_ROOT)

from config.species_config import DATA_DIR, BASE_ROOT, SKIP_DIRS, resolve_pfam_paths
...
def resolve_output_paths(pfam_id, output_root):
    ...
    Mirrors the logic from config.species_config.resolve_pfam_paths.
    """
...
def main():
    ...
    print(f"\nNext step: python scripts/02_extract_domain_seqs.py {pfam_id}")
    print("=" * 60)
